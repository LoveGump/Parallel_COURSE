// SIMD+OpenMP Montgomery版本 NTT多项式乘法
#include <omp.h>
#include <sys/time.h>
...
#include "Montgomery.h"  // 使用现有的Montgomery实现
...
// Montgomery 对象全局实例
Montgomery *GLOBAL_MONTGOMERY_LIST[4] = {nullptr, nullptr, nullptr, nullptr};
...
/**
 * @brief SIMD版本的蝶形运算 - 使用AVX2指令集和Montgomery约化
 * @param u_ptr 指向u值的指针
 * @param v_ptr 指向v值的指针  
 * @param len 处理长度
 * @param montgomery Montgomery约化器
 */
inline void simd_butterfly_avx2_montgomery(uint64_t *u_ptr, uint64_t *v_ptr, int len, const Montgomery &montgomery) {
    const uint64_t mod = montgomery.N;
...
/**
 * @brief SIMD+OpenMP优化的NTT变换函数 - 使用Montgomery约化
 * @param a 输入数组
 * @param n 数组长度
 ...
 * @param g 原根
 */
void NTT_simd_openmp_montgomery(uint64_t *a, uint64_t n, bool invert, const Montgomery &montgomery, int num_threads = NUM_THREADS, int g = 3) {
    uint64_t p = montgomery.N;
    
    for (int len = 2; len <= n; len <<= 1) {
        uint64_t g_n = invert ? Montgomery::pow(g, (p - 1) - (p - 1) / len, p)
                              : Montgomery::pow(g, (p - 1) / len, p);
        g_n = montgomery.REDC(g_n * montgomery.R2);  // 转换到Montgomery域
        
        int step = len >> 1;
        
#pragma omp parallel for schedule(static) num_threads(num_threads)
        for (int i = 0; i < n; i += len) {
            uint64_t gk = montgomery.R_mon_N;  // Montgomery域中的1
            ...
            for (int j = 0; j < step; j += 4) {
                ...
                if (end - j == 4) {
                    ...
                    for (int k = 0; k < 4; k++) {
                        gk_vals[k] = gk;
                        gk = montgomery.REDC(gk * g_n);  // Montgomery域乘法
                    }
                    
                    // 使用Montgomery约化进行模乘运算
                    ...
                    for (int k = 0; k < 4; k++) {
                        uint64_t u = u_vals[k];
                        uint64_t v = montgomery.REDC(v_vals[k] * gk_vals[k]);  // Montgomery域乘法
                        uint64_t sum = u + v;
                        if (sum >= p) sum -= p;
                    ...
                    for (int k = j; k < end; k++) {
                        uint64_t u = a[i + k];
                        uint64_t v = montgomery.REDC(a[i + k + step] * gk);  // Montgomery域乘法
                        uint64_t sum = u + v;
                        if (sum >= p) sum -= p;
                        uint64_t diff = u >= v ? u - v : u + p - v;
                        a[i + k] = sum;
    ...
    if (invert) {
        uint64_t inv_n = Montgomery::pow(n, p - 2, p);
        inv_n = montgomery.REDC(inv_n * montgomery.R2);  // 转换到Montgomery域
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
        for (int i = 0; i < n; i += 4) {
            ...
            if (end - i == 4) {
                ...
                for (int j = 0; j < 4; j++) {
                    a_vals[j] = montgomery.REDC(a_vals[j] * inv_n);  // Montgomery域乘法
                }
                ...
                // 处理剩余元素
                for (int j = i; j < end; j++) {
                    a[j] = montgomery.REDC(a[j] * inv_n);  // Montgomery域乘法
                }
            }
        }
    }
...
/**
 * @brief SIMD+OpenMP优化的点值乘法 - 使用Montgomery约化
 * @param fa 多项式A的点值表示
 * @param fb 多项式B的点值表示
 * @param len 长度
 * @param montgomery Montgomery约化器
 * @param num_threads 线程数
 */
void pointwise_multiply_simd_openmp_montgomery(uint64_t *fa, uint64_t *fb, int len, const Montgomery &montgomery, int num_threads = NUM_THREADS) {
#pragma omp parallel for schedule(static) num_threads(num_threads)
    for (int i = 0; i < len; i += 4) {
        ...
        if (end - i == 4) {
            ...
            // 处理剩余元素
            for (int j = i; j < end; j++) {
                fa[j] = montgomery.REDC(fa[j] * fb[j]);  // Montgomery域乘法
            }
        }
    }
}
...
/**
 * @brief 使用SIMD+OpenMP和Montgomery约化的NTT多项式乘法
 * @param a 输入多项式A
 * @param b 输入多项式B
 * @param result 结果数组
 * @param n 多项式长度
 ...
 * @param threads 线程数
 */
void NTT_multiply_simd_openmp_montgomery(uint64_t *a, uint64_t *b, uint64_t *result, int n, uint64_t p, int threads = NUM_THREADS) {
    Montgomery montgomery(p);
    
    int len = (n << 1);
    uint64_t *fa = new uint64_t[len];
    uint64_t *fb = new uint64_t[len];
    
    // SIMD+OpenMP优化的数组初始化，转换到Montgomery域
#pragma omp parallel for schedule(static) num_threads(threads)
    for (int i = 0; i < n; i += 4) {
        ...
        if (end - i == 4) {
            ...
            for (int j = 0; j < 4; j++) {
                a_vals[j] = montgomery.REDC(a[i + j] * montgomery.R2);  // 转换到Montgomery域
                b_vals[j] = montgomery.REDC(b[i + j] * montgomery.R2);  // 转换到Montgomery域
            }
            ...
            for (int j = i; j < end; j++) {
                fa[j] = montgomery.REDC(a[j] * montgomery.R2);  // 转换到Montgomery域
                fb[j] = montgomery.REDC(b[j] * montgomery.R2);  // 转换到Montgomery域
            }
        }
    }
...
#pragma omp parallel for schedule(static) num_threads(threads)
    ...
    NTT_simd_openmp_montgomery(fa, len, false, montgomery, threads);
    NTT_simd_openmp_montgomery(fb, len, false, montgomery, threads);
    
    // SIMD+OpenMP优化的点值乘法
    pointwise_multiply_simd_openmp_montgomery(fa, fb, len, montgomery, threads);
    
    // 逆NTT
    reverse_simd_openmp(fa, len, bit, rev, threads);
    NTT_simd_openmp_montgomery(fa, len, true, montgomery, threads);
    
    // 复制结果，从Montgomery域转换回普通域
#pragma omp parallel for schedule(static) num_threads(threads)
    for (int i = 0; i < (n << 1) - 1; i++) {
        result[i] = montgomery.REDC(fa[i]);  // 从Montgomery域转换回普通域
    }
...
void init_global_crt_values() {
    ...
    for (int i = 0; i < GLOBAL_MOD_COUNT; i++) {
        ...
        // 使用Montgomery约化计算逆元
        Montgomery montgomery(mod_num);
        GLOBAL_MI_INV_VALUES[i] = Montgomery::pow(Mi_mod, mod_num - 2, mod_num);
        
        // 初始化Montgomery对象
        GLOBAL_MONTGOMERY_LIST[i] = new Montgomery(mod_num);
    }
...
void cleanup_global_crt_values() {
    for (int i = 0; i < GLOBAL_MOD_COUNT; i++) {
        ...
        if (GLOBAL_MONTGOMERY_LIST[i]) {
            delete GLOBAL_MONTGOMERY_LIST[i];
            GLOBAL_MONTGOMERY_LIST[i] = nullptr;
        }
    }
}

/**
 * @brief 为CRT服务的SIMD+OpenMP多项式乘法 - 使用Montgomery约化
 */
void NTT_multiply_simd_openmp_big_montgomery(uint64_t *a, uint64_t *b, uint64_t *result, int n, uint64_t p) {
    Montgomery montgomery(p);
    ...
    // SIMD+OpenMP优化的数组初始化和模运算，转换到Montgomery域
#pragma omp parallel for schedule(static) num_threads(inner_threads)
    for (int i = 0; i < n; i += 4) {
        ...
        if (end - i == 4) {
            ...
            uint64_t a_vals[4], b_vals[4];
            for (int j = 0; j < 4; j++) {
                uint64_t a_mod = a[i + j] % p;
                uint64_t b_mod = b[i + j] % p;
                a_vals[j] = montgomery.REDC(a_mod * montgomery.R2);  // 转换到Montgomery域
                b_vals[j] = montgomery.REDC(b_mod * montgomery.R2);  // 转换到Montgomery域
            }
            __m256i a_vec = _mm256_loadu_si256((__m256i*)a_vals);
            __m256i b_vec = _mm256_loadu_si256((__m256i*)b_vals);
            _mm256_storeu_si256((__m256i*)(fa + i), a_vec);
            ...
            for (int j = i; j < end; j++) {
                uint64_t a_mod = a[j] % p;
                uint64_t b_mod = b[j] % p;
                fa[j] = montgomery.REDC(a_mod * montgomery.R2);  // 转换到Montgomery域
                fb[j] = montgomery.REDC(b_mod * montgomery.R2);  // 转换到Montgomery域
            }
        }
    }
...
#pragma omp parallel for schedule(static) num_threads(inner_threads)
    ...
    NTT_simd_openmp_montgomery(fa, len, false, montgomery, inner_threads);
    NTT_simd_openmp_montgomery(fb, len, false, montgomery, inner_threads);
    pointwise_multiply_simd_openmp_montgomery(fa, fb, len, montgomery, inner_threads);
    reverse_simd_openmp(fa, len, bit, rev, inner_threads);
    NTT_simd_openmp_montgomery(fa, len, true, montgomery, inner_threads);
...
/**
 * @brief 使用SIMD+OpenMP和Montgomery约化的CRT多项式乘法
 */
void CRT_NTT_multiply_simd_openmp_montgomery(uint64_t *a, uint64_t *b, uint64_t *result, int n, uint64_t p) {
...
#pragma omp parallel for num_threads(GLOBAL_MOD_COUNT) schedule(static)
    for (int i = 0; i < GLOBAL_MOD_COUNT; i++) {
        NTT_multiply_simd_openmp_big_montgomery(a, b, GLOBAL_MOD_RESULTS[i], n, GLOBAL_MOD_LIST[i]);
    }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = test_begin; i <= test_end; ++i) {
        ...
        // 根据模数大小选择算法
        if (p_ > (1ULL << 32)) {
            CRT_NTT_multiply_simd_openmp_montgomery(a, b, ab, n_, p_);
        } else {
            NTT_multiply_simd_openmp_montgomery(a, b, ab, n_, p_);
        }
        ...
        fCheck(ab, n_, i);
        std::cout << "SIMD+OpenMP Montgomery - average latency for n = " << n_ << " p = " << p_ << " : "
                  << ans << " (ms) " << std::endl;
        fWrite(ab, n_, i);
    }
