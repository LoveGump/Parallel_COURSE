// SIMD+OpenMP Barrett版本 NTT多项式乘法
#include <omp.h>
#include <sys/time.h>
...
struct Barrett {
    uint64_t mod;
    __uint128_t im;
    Barrett(uint64_t m) : mod(m), im(((__uint128_t)1 << 64) / m) {}
    uint64_t reduce(uint64_t a) const {
        __uint128_t q = ((__uint128_t)a * im) >> 64;
        uint64_t r = a - q * mod;
...
// Barrett 对象全局实例
Barrett *GLOBAL_BARRETT_LIST[4] = {nullptr, nullptr, nullptr, nullptr};
 ...
 * @param base 底数
 * @param exp 指数
 * @param barrett Barrett约化器
 */
uint64_t pow_barrett(uint64_t base, uint64_t exp, const Barrett &barrett) {
    uint64_t res = 1;
    base = barrett.reduce(base);
    while (exp > 0) {
        if (exp & 1) {
            res = barrett.mul(res, base);
        }
        base = barrett.mul(base, base);
        exp >>= 1;
    }
    return res;
}
...
/**
 * @brief SIMD版本的蝶形运算 - 使用AVX2指令集和Barrett约化
 * @param u_ptr 指向u值的指针
 * @param v_ptr 指向v值的指针  
 * @param len 处理长度
 * @param barrett Barrett约化器
 */
inline void simd_butterfly_avx2_barrett(uint64_t *u_ptr, uint64_t *v_ptr, int len, const Barrett &barrett) {
    const uint64_t mod = barrett.mod;
...
/**
 * @brief SIMD+OpenMP优化的NTT变换函数 - 使用Barrett约化
 * @param a 输入数组
 * @param n 数组长度
 ...
 * @param g 原根
 */
void NTT_simd_openmp_barrett(uint64_t *a, uint64_t n, bool invert, const Barrett &barrett, int num_threads = NUM_THREADS, int g = 3) {
    uint64_t p = barrett.mod;
    
    for (int len = 2; len <= n; len <<= 1) {
        uint64_t g_n = invert ? pow_barrett(g, (p - 1) - (p - 1) / len, barrett)
                              : pow_barrett(g, (p - 1) / len, barrett);
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
        for (int i = 0; i < n; i += len) {
            ...
            for (int j = 0; j < step; j += 4) {
                ...
                if (end - j == 4) {
                    ...
                    for (int k = 0; k < 4; k++) {
                        uint64_t u = u_vals[k];
                        uint64_t v = barrett.mul(v_vals[k], gk_vals[k]);
                        uint64_t sum = u + v;
                        if (sum >= p) sum -= p;
                    ...
                    for (int k = j; k < end; k++) {
                        uint64_t u = a[i + k];
                        uint64_t v = barrett.mul(a[i + k + step], gk);
                        uint64_t sum = u + v;
                        if (sum >= p) sum -= p;
                        uint64_t diff = u >= v ? u - v : u + p - v;
                        a[i + k] = sum;
    ...
    if (invert) {
        uint64_t inv_n = pow_barrett(n, p - 2, barrett);
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
        for (int i = 0; i < n; i += 4) {
            ...
            if (end - i == 4) {
                ...
                for (int j = 0; j < 4; j++) {
                    a_vals[j] = barrett.mul(a_vals[j], inv_n);
                }
...
/**
 * @brief SIMD+OpenMP优化的点值乘法 - 使用Barrett约化
 * @param fa 多项式A的点值表示
 * @param fb 多项式B的点值表示
 * @param len 长度
 * @param barrett Barrett约化器
 * @param num_threads 线程数
 */
void pointwise_multiply_simd_openmp_barrett(uint64_t *fa, uint64_t *fb, int len, const Barrett &barrett, int num_threads = NUM_THREADS) {
#pragma omp parallel for schedule(static) num_threads(num_threads)
...
/**
 * @brief 使用SIMD+OpenMP和Barrett约化的NTT多项式乘法
 * @param a 输入多项式A
 * @param b 输入多项式B
 * @param result 结果数组
 * @param n 多项式长度
 ...
 * @param threads 线程数
 */
void NTT_multiply_simd_openmp_barrett(uint64_t *a, uint64_t *b, uint64_t *result, int n, uint64_t p, int threads = NUM_THREADS) {
    Barrett barrett(p);
...
#pragma omp parallel for schedule(static) num_threads(threads)
    ...
    NTT_simd_openmp_barrett(fa, len, false, barrett, threads);
    NTT_simd_openmp_barrett(fb, len, false, barrett, threads);
    
    // SIMD+OpenMP优化的点值乘法
    pointwise_multiply_simd_openmp_barrett(fa, fb, len, barrett, threads);
    
    // 逆NTT
    reverse_simd_openmp(fa, len, bit, rev, threads);
    NTT_simd_openmp_barrett(fa, len, true, barrett, threads);
...
void init_global_crt_values() {
    ...
    for (int i = 0; i < GLOBAL_MOD_COUNT; i++) {
        ...
        // 使用Barrett约化计算逆元
        Barrett barrett(mod_num);
        GLOBAL_MI_INV_VALUES[i] = pow_barrett(Mi_mod, mod_num - 2, barrett);
        
        // 初始化Barrett对象
        GLOBAL_BARRETT_LIST[i] = new Barrett(mod_num);
    }
...
void cleanup_global_crt_values() {
    for (int i = 0; i < GLOBAL_MOD_COUNT; i++) {
        ...
        if (GLOBAL_BARRETT_LIST[i]) {
            delete GLOBAL_BARRETT_LIST[i];
            GLOBAL_BARRETT_LIST[i] = nullptr;
        }
    }
}

/**
 * @brief 为CRT服务的SIMD+OpenMP多项式乘法 - 使用Barrett约化
 */
void NTT_multiply_simd_openmp_big_barrett(uint64_t *a, uint64_t *b, uint64_t *result, int n, uint64_t p) {
    Barrett barrett(p);
...
#pragma omp parallel for schedule(static) num_threads(inner_threads)
    for (int i = 0; i < n; i += 4) {
        ...
        if (end - i == 4) {
            ...
            for (int j = 0; j < 4; j++) {
                a_vals[j] = barrett.reduce(a[i + j]);
                b_vals[j] = barrett.reduce(b[i + j]);
            }
            ...
            for (int j = i; j < end; j++) {
                fa[j] = barrett.reduce(a[j]);
                fb[j] = barrett.reduce(b[j]);
            }
        }
    }
...
#pragma omp parallel for schedule(static) num_threads(inner_threads)
    ...
    NTT_simd_openmp_barrett(fa, len, false, barrett, inner_threads);
    NTT_simd_openmp_barrett(fb, len, false, barrett, inner_threads);
    pointwise_multiply_simd_openmp_barrett(fa, fb, len, barrett, inner_threads);
    reverse_simd_openmp(fa, len, bit, rev, inner_threads);
    NTT_simd_openmp_barrett(fa, len, true, barrett, inner_threads);
...
/**
 * @brief 使用SIMD+OpenMP和Barrett约化的CRT多项式乘法
 */
void CRT_NTT_multiply_simd_openmp_barrett(uint64_t *a, uint64_t *b, uint64_t *result, int n, uint64_t p) {
...
#pragma omp parallel for num_threads(GLOBAL_MOD_COUNT) schedule(static)
    for (int i = 0; i < GLOBAL_MOD_COUNT; i++) {
        NTT_multiply_simd_openmp_big_barrett(a, b, GLOBAL_MOD_RESULTS[i], n, GLOBAL_MOD_LIST[i]);
    }
...
#pragma omp parallel for num_threads(NUM_THREADS) schedule(static)
    for (int j = 0; j < result_len; j += 4) {
        ...
        if (end - j == 4) {
            ...
            for (int k = 0; k < 4; k++) {
                __uint128_t term0 = GLOBAL_MI_VALUES[0] * (GLOBAL_BARRETT_LIST[0]->mul(GLOBAL_MOD_RESULTS[0][j + k], GLOBAL_MI_INV_VALUES[0]));
                __uint128_t term1 = GLOBAL_MI_VALUES[1] * (GLOBAL_BARRETT_LIST[1]->mul(GLOBAL_MOD_RESULTS[1][j + k], GLOBAL_MI_INV_VALUES[1]));
                __uint128_t term2 = GLOBAL_MI_VALUES[2] * (GLOBAL_BARRETT_LIST[2]->mul(GLOBAL_MOD_RESULTS[2][j + k], GLOBAL_MI_INV_VALUES[2]));
                __uint128_t term3 = GLOBAL_MI_VALUES[3] * (GLOBAL_BARRETT_LIST[3]->mul(GLOBAL_MOD_RESULTS[3][j + k], GLOBAL_MI_INV_VALUES[3]));
            ...
            // 处理剩余元素
            for (int k = j; k < end; k++) {
                __uint128_t term0 = GLOBAL_MI_VALUES[0] * (GLOBAL_BARRETT_LIST[0]->mul(GLOBAL_MOD_RESULTS[0][k], GLOBAL_MI_INV_VALUES[0]));
                __uint128_t term1 = GLOBAL_MI_VALUES[1] * (GLOBAL_BARRETT_LIST[1]->mul(GLOBAL_MOD_RESULTS[1][k], GLOBAL_MI_INV_VALUES[1]));
                __uint128_t term2 = GLOBAL_MI_VALUES[2] * (GLOBAL_BARRETT_LIST[2]->mul(GLOBAL_MOD_RESULTS[2][k], GLOBAL_MI_INV_VALUES[2]));
                __uint128_t term3 = GLOBAL_MI_VALUES[3] * (GLOBAL_BARRETT_LIST[3]->mul(GLOBAL_MOD_RESULTS[3][k], GLOBAL_MI_INV_VALUES[3]));
...
int main(int argc, char *argv[]) {
    ...
    for (int i = test_begin; i <= test_end; ++i) {
        ...
        // 根据模数大小选择算法
        if (p_ > (1ULL << 32)) {
            CRT_NTT_multiply_simd_openmp_barrett(a, b, ab, n_, p_);
        } else {
            NTT_multiply_simd_openmp_barrett(a, b, ab, n_, p_);
        }
        ...
        fCheck(ab, n_, i);
        std::cout << "SIMD+OpenMP Barrett - average latency for n = " << n_ << " p = " << p_ << " : "
                  << ans << " (ms) " << std::endl;
        fWrite(ab, n_, i);
    }
